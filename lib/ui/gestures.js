...
export function attachPanZoom(scrollEl, opts = {}) {
  ...
  // Idempotent: callers like Slides re-run bootstrap() (and thus this)
  // on every document-tab switch. Without this guard each call would
  // stack another listener set on the same element, so one gesture
  // would fire onPinch/onPan once per past attach.
  if (scrollEl.__rwPanZoomBound) {
    return scrollEl.__rwPanZoomDetach || (() => {});
  }
  scrollEl.__rwPanZoomBound = true;
  ...
  const detach = function detach() {
    ...
    scrollEl.__rwPanZoomBound = false;
    scrollEl.__rwPanZoomDetach = null;
  };
  scrollEl.__rwPanZoomDetach = detach;
  return detach;
}
