...
function setOrInsertFlag(args, flag, value) {
  ...
  // No -c:v in the base args (e.g. an `-an` video-only encoder, or a
  // future audio-only target). Append at the end so the flag lands
  // right before the output filename when transcode() joins them,
  // which is FFmpeg's conventional per-stream option position.
  out.push(flag, value);
  return out;
}
 ...
 *
 * `options` (all fields optional) lets the caller override the
 * static ENCODER_FOR defaults with user-driven choices:
 *
 *   - `videoPreset`:     'low' | 'medium' | 'high' | 'original'
 *     Picks a bitrate + scale-height bundle (see VIDEO_PRESETS).
 *   - `videoResolution`: 'auto' | '480p' | '720p' | '1080p'
 *     Overrides the preset's scale-height. 'auto' inherits the preset.
 *   - `videoBitrate`:    string like '1500k'. Overrides preset bitrate.
 *   - `audioCodec`:      'aac' | 'mp3' | 'opus' | 'vorbis' | 'copy'.
 *     Swaps the per-container default audio codec.
 *   - `audioBitrate`:    string like '128k'.
 *
 ...
 *
 * `options.audioOnlyBitrate` (string like '128k') overrides the
 * encoder's default `-b:a` / `-q:a` for the audio-only path. Audio
 * codec selection isn't user-tunable here because each audio target
 * is codec-specific by definition (`.mp3` ⇒ libmp3lame, `.flac` ⇒
 * flac, etc.). Use the video path with `audioCodec` to swap codecs.
 *
 * @param {Uint8Array} bytes
 * @param {{
 *   from: string,
 *   to: string,
 *   onProgress?: (ratio: number) => void,
 *   options?: { audioOnlyBitrate?: string },
 * }} opts
 * @returns {Promise<Uint8Array>}
 */
