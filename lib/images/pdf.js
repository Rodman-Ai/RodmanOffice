...
//  compressPdf         — rasterize every page at a level-driven JPEG
//                        quality + scale (5 presets from "minimum"
//                        through "maximum") and reassemble. With
//                        `preserveText: true` (default), an invisible
//                        text overlay rebuilt from pdf.js
//                        getTextContent keeps Cmd-F / select working.
// =============================================================
  ...
  try {
    ...
    const items = tc.items.map((it) => {
      ...
      return {
        str: it.str,
        x: t[4],
        // PDF user space — origin bottom-left, +Y up. Same convention as
        // the Tm operator we emit downstream, so t[5] flows through
        // verbatim. (Earlier code subtracted from viewport.height,
        // which inverted the Y axis and put every line at the bottom
        // of the page.)
        y: t[5],
