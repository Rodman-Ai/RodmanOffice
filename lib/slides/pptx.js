...
function parseSlide(xmlText, slideRels, files, slidePath, slideIdx) {
  ...
  function emitShape(node) {
    // Placeholders (title, body, subtitle) usually carry no position of
    // their own and inherit it from the slide layout or master.
    const xfrm = readXfrm(node) || placeholderXfrm(node, files, slideRels, slidePath);
    if (!xfrm) return;
...
const layoutDocs = new WeakMap(); // files -> Map(path -> parsed XML or null)

function resolvePartPath(base, target) {
  return new URL(target, 'http://pptx/' + base).pathname.slice(1);
}

function readPart(files, path) {
  let cache = layoutDocs.get(files);
  if (!cache) layoutDocs.set(files, (cache = new Map()));
  if (!cache.has(path)) {
    let doc = null;
    try { if (files[path]) doc = parseXml(dec.decode(files[path])); } catch { doc = null; }
    cache.set(path, doc);
  }
  return cache.get(path);
}

function relTargetOfType(files, partPath, word) {
  const relsPath = partPath.replace(/\/([^/]+)$/, '/_rels/$1.rels');
  if (!files[relsPath]) return null;
  const doc = readPart(files, relsPath);
  const rel = doc && Array.from(doc.getElementsByTagNameNS('*', 'Relationship'))
    .find((el) => (el.getAttribute('Type') || '').endsWith('/' + word));
  return rel ? resolvePartPath(partPath, rel.getAttribute('Target')) : null;
}

function phOf(node) {
  const nvPr = Array.from(node.getElementsByTagNameNS('*', 'nvPr'))[0];
  const ph = nvPr && nvPr.getElementsByTagNameNS('*', 'ph')[0];
  if (!ph) return null;
  return { type: ph.getAttribute('type') || 'body', idx: ph.getAttribute('idx') };
}

// Finds the matching placeholder's position in the slide's layout, then
// its master. Falls back to a standard title or body box.
function placeholderXfrm(node, files, slideRels, slidePath) {
  const ph = phOf(node);
  if (!ph) return null;
  const isTitle = PH_TITLE.has(ph.type);
  const layoutPath = relTargetOfType(files, slidePath, 'slideLayout');
  const masterPath = layoutPath && relTargetOfType(files, layoutPath, 'slideMaster');
  for (const path of [layoutPath, masterPath]) {
    const doc = path && readPart(files, path);
    if (!doc) continue;
    const shapes = Array.from(doc.getElementsByTagNameNS('*', 'sp'));
    const match = shapes.find((sp) => {
      const p = phOf(sp);
      return p && ph.idx != null && p.idx === ph.idx;
    }) || shapes.find((sp) => {
      const p = phOf(sp);
      if (!p) return false;
      if (isTitle) return PH_TITLE.has(p.type);
      return path === masterPath ? p.type === 'body' : p.type === ph.type;
    });
    const xfrm = match && readXfrm(match);
    if (xfrm) return xfrm;
  }
  return isTitle
    ? { x: 64, y: 40, w: SLIDE_W_PX - 128, h: 110 }
    : { x: 64, y: 170, w: SLIDE_W_PX - 128, h: SLIDE_H_PX - 230 };
}
