...
//
// `delim` lets the same RFC-4180 quoting machinery serve TSV, PSV,
// and any other single-character delimited variant. Defaults to ','
// so existing callers (importCsv / exportSheetAsCSV / AiCell codecs)
// keep their behavior.
...
function unparseCsv(rows, delim) {
  const d = delim || ',';
  // Quote any field containing the delimiter, a quote, or a line break.
...
// ---------- Low-level XLSX parser / builder (Sheet2D matrices) ----------
...
function parseXlsx(buf) {
  ...
  for (const name of wb.SheetNames) {
    ...
    if (!ws) continue;
    const raw = XLSX.utils.sheet_to_json(ws, { header: 1, raw: false, blankrows: true });
    const rows = raw.map((row) => (row ?? []).map((v) => (v == null ? '' : String(v))));
    out.push({ name, rows });
  }
  return out;
}
...
export { parseCsv, unparseCsv, parseXlsx, buildXlsx };
...
export function exportSheetAsCSV(sheet) {
  ...
  return new TextEncoder().encode(unparseCsv(rows));
}
...
/**
 * Serialize a workbook to legacy Excel 97-2003 (BIFF8 / .xls) bytes.
 * Same builder pipeline as XLSX, just a different bookType — the
 * vendored xlsx engine writes both.
 * @param {import('./types.js').Workbook} workbook
 * @returns {Uint8Array}
 */
export function exportWorkbookAsXLS(workbook) {
  const wb = XLSX.utils.book_new();
  for (const sheet of workbook.sheets) {
    const rows = sheetToMatrix(sheet);
    const ws = XLSX.utils.aoa_to_sheet(rows.length > 0 ? rows : [[]]);
