...
function assertZipLimit(condition, message) {
  if (!condition) throw new Error(`ZIP import rejected: ${message}`);
}
...
async function readZip(buffer) {
  ...
  assertZipLimit(u8.length <= ZIP_LIMITS.maxArchiveBytes, `archive exceeds ${ZIP_LIMITS.maxArchiveBytes} bytes`);
  ...
  assertZipLimit(cdEntries <= ZIP_LIMITS.maxEntries, `archive has too many entries (${cdEntries})`);

  const files = {};
  let totalUncompressed = 0;
  for (let i = 0; i < cdEntries; i++) {
    assertZipLimit(cdOff + 46 <= u8.length, 'central directory is truncated');
    if (view.getUint32(cdOff, true) !== 0x02014b50) throw new Error('Invalid ZIP central directory');
    ...
    assertZipLimit(nameLen <= ZIP_LIMITS.maxNameBytes, `entry name exceeds ${ZIP_LIMITS.maxNameBytes} bytes`);
    assertZipLimit(uncompressedSize <= ZIP_LIMITS.maxEntryBytes, `entry exceeds ${ZIP_LIMITS.maxEntryBytes} bytes`);
    assertZipLimit(cdOff + 46 + nameLen + extraLen + commentLen <= u8.length, 'central directory entry is truncated');
    if (compressedSize > 0 && uncompressedSize > compressedSize * ZIP_LIMITS.maxCompressionRatio) {
      throw new Error(`ZIP import rejected: entry compression ratio exceeds ${ZIP_LIMITS.maxCompressionRatio}:1`);
    }
    const name = dec.decode(u8.subarray(cdOff + 46, cdOff + 46 + nameLen));
    cdOff += 46 + nameLen + extraLen + commentLen;

    // Local file header
    assertZipLimit(localOff + 30 <= u8.length, `local header for "${name}" is truncated`);
    assertZipLimit(view.getUint32(localOff, true) === 0x04034b50, `local header for "${name}" is invalid`);
    const lhNameLen = view.getUint16(localOff + 26, true);
    const lhExtraLen = view.getUint16(localOff + 28, true);
    const dataStart = localOff + 30 + lhNameLen + lhExtraLen;
    assertZipLimit(dataStart + compressedSize <= u8.length, `entry data for "${name}" is truncated`);
    ...
    assertZipLimit(bytes.length <= ZIP_LIMITS.maxEntryBytes, `entry "${name}" exceeds ${ZIP_LIMITS.maxEntryBytes} bytes after decompression`);
    totalUncompressed += bytes.length;
    assertZipLimit(totalUncompressed <= ZIP_LIMITS.maxTotalBytes, `archive expands beyond ${ZIP_LIMITS.maxTotalBytes} bytes`);
    files[name] = bytes;
  }
  return files;
}
...
// ---------- Public API ----------
function saveDocx(html, opts) {
  return new Blob([buildDocx(html, opts)], {
    type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
  });
}

export { saveDocx, loadDocx, buildZip, readZip };
