...
function assemblePdf(pages, opts) {
  ...
  pages.forEach((p, idx) => {
    ...
    // The file is written one byte per character (Latin-1, below), so the
    // character count is the byte count. A UTF-8 count overstated it for
    // every accented or typographic character.
  ...
  // Concat with byte offsets. Objects are written in a different order
  // than they are numbered (all pages, then all content streams), so each
  // offset is recorded under its object's own number.
  const header = '%PDF-1.4\n%\xE2\xE3\xCF\xD3\n';
  const parts = [catalogStr, pagesStr, ...pageStrs, ...contentsStrs, ...fontStrs, infoStr];
  let body = header;
  const offsets = [0]; // index by object number; placeholder for 0
  parts.forEach((p) => {
    offsets[parseInt(p, 10)] = body.length;
    body += p;
  });
