...
//  encryption) and pass them back as an image source:
//
//    { images: [{ jpeg: Uint8Array, width, height, pageWidth,
//                 pageHeight }] }
//
//  Other source shape:
...
//  named destinations, AcroForm, optional-content (layer) settings,
//  structure tree (tagging), attachments and document JavaScript.
//  So links that go through named destinations, or to pages that
//  were left out, stop working; form fields keep their appearance
//  but are no longer fillable; and content on layers that the
//  source hid by default can show up.
// =============================================================
...
async function copyDocumentPages(reader, src, writer, pagesRef, onPage) {
  ...
  const copyValue = async (v) => {
    if (v instanceof PdfRef) {
      ...
      const target = await reader.getObject(v.num);
...
function imagePages(src, writer, pagesRef, onPage) {
  ...
  for (const img of src.images) {
    const imgRef = writer.add(new PdfStream(
      new PdfDict()
        .set('Type', new PdfName('XObject'))
        .set('Subtype', new PdfName('Image'))
        .set('Width', img.width)
        .set('Height', img.height)
        .set('ColorSpace', new PdfName('DeviceRGB'))
        .set('BitsPerComponent', 8)
        .set('Filter', new PdfName('DCTDecode')),
      img.jpeg,
    ));
    const w = img.pageWidth, h = img.pageHeight;
    const content = writer.add(new PdfStream(new PdfDict(), latin1Bytes(`q ${fmtNum(w)} 0 0 ${fmtNum(h)} 0 0 cm /Im0 Do Q`)));
    const page = new PdfDict()
 ...
 * @param {{ bookmarks?: boolean, title?: string, onProgress?: (ratio: number) => void }} [opts]
