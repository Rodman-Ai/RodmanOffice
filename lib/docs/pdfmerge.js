...
async function decodeStream(stream) {
  ...
  for (let i = 0; i < filters.length; i++) {
    const f = filters[i]?.name;
    if (f === 'FlateDecode' || f === 'Fl') data = unpredict(await inflate(data), parms[i]);
...
export class PdfReader {
  ...
  async fromObjStm(stmNum, idx, n) {
    ...
    if (!entry) {
      ...
      const data = await decodeStream(stm);
...
export function parsePageRanges(text, pageCount) {
  ...
  for (const part of t.split(/[,;\s]+/).filter(Boolean)) {
    ...
    if (a < 1 || b < 1 || a > pageCount || b > pageCount) throw new Error(`Page ${Math.max(a, b)} is out of range (1-${pageCount})`);
    const step = a <= b ? 1 : -1;
