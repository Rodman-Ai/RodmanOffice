...
// ODP (OpenDocument Presentation) — minimal package, ZIP-based.
// One <draw:page> per H1-bounded chunk; the chunk's body becomes a
// single text frame. Reuses the stored-only buildZip writer that
// already powers ODT and ODS in this suite.
function buildOdp(html, title) {
  const tmp = document.createElement('div');
  tmp.innerHTML = html;

  // Split children into slides at H1 boundaries.
  const slides = [];
  let bucket = { title: title || 'Slide', body: '' };
  Array.from(tmp.childNodes).forEach((n) => {
    if (n.nodeType === 1 && n.tagName === 'H1') {
      if (bucket.body || slides.length === 0) slides.push(bucket);
      bucket = { title: n.textContent || 'Slide', body: '' };
    } else {
      bucket.body += n.outerHTML || (n.nodeValue || '');
    }
  });
  if (bucket.body || !slides.length) slides.push(bucket);
  ...
  function odpInline(node) {
    if (node.nodeType === 3) return escXml(node.nodeValue);
    if (node.nodeType !== 1) return '';
    let inner = '';
    node.childNodes.forEach((c) => { inner += odpInline(c); });
    const tag = node.tagName.toLowerCase();
    if (tag === 'b' || tag === 'strong') return `<text:span text:style-name="Bold">${inner}</text:span>`;
    if (tag === 'i' || tag === 'em') return `<text:span text:style-name="Italic">${inner}</text:span>`;
    return inner;
  }
  function odpBlocks(htmlString) {
    ...
    Array.from(frag.childNodes).forEach((n) => {
      if (n.nodeType === 3) {
        const t = n.nodeValue.trim();
        if (t) xml += `<text:p>${escXml(t)}</text:p>`;
        return;
      }
      if (n.nodeType !== 1) return;
      const tag = n.tagName.toLowerCase();
      if (/^h[1-6]$/.test(tag)) xml += `<text:p>${odpInline(n)}</text:p>`;
      else if (tag === 'p') xml += `<text:p>${odpInline(n)}</text:p>`;
      else if (tag === 'ul' || tag === 'ol') {
        xml += '<text:list>';
        Array.from(n.querySelectorAll(':scope > li')).forEach((li) => {
          xml += `<text:list-item><text:p>${odpInline(li)}</text:p></text:list-item>`;
        });
        xml += '</text:list>';
      } else {
        xml += `<text:p>${odpInline(n)}</text:p>`;
      }
    });
    return xml;
  }
  ...
  slides.forEach((slide, idx) => {
    pages +=
      `<draw:page draw:name="page${idx + 1}" draw:style-name="dp1" draw:master-page-name="Default">` +
        `<draw:frame draw:style-name="title" presentation:class="title" svg:width="24cm" svg:height="3cm" svg:x="2cm" svg:y="1.5cm">` +
          `<draw:text-box><text:p>${escXml(slide.title)}</text:p></draw:text-box>` +
        `</draw:frame>` +
        `<draw:frame draw:style-name="body" presentation:class="outline" svg:width="24cm" svg:height="13cm" svg:x="2cm" svg:y="5cm">` +
          `<draw:text-box>${odpBlocks(slide.body)}</draw:text-box>` +
        `</draw:frame>` +
      `</draw:page>`;
  });
  ...
  const content = '<?xml version="1.0" encoding="UTF-8"?>' +
    `<office:document-content${NS} office:version="1.2">` +
      '<office:automatic-styles>' +
        '<style:style style:name="Bold" style:family="text" xmlns:style="urn:oasis:names:tc:opendocument:xmlns:style:1.0"><style:text-properties fo:font-weight="bold" xmlns:fo="urn:oasis:names:tc:opendocument:xmlns:xsl-fo-compatible:1.0"/></style:style>' +
        '<style:style style:name="Italic" style:family="text" xmlns:style="urn:oasis:names:tc:opendocument:xmlns:style:1.0"><style:text-properties fo:font-style="italic" xmlns:fo="urn:oasis:names:tc:opendocument:xmlns:xsl-fo-compatible:1.0"/></style:style>' +
      '</office:automatic-styles>' +
  ...
  return buildZip([
    { name: 'mimetype', data: enc.encode('application/vnd.oasis.opendocument.presentation') },
    { name: 'content.xml', data: enc.encode(content) },
    { name: 'META-INF/manifest.xml', data: enc.encode(manifest) },
  ]);
}
