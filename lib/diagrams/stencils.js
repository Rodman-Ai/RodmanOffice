// The 132-shape stencil catalog across 16 categories.
//
...
// Clamp an SVG radius to a small positive minimum. BPMN event
// circles compute their radius as min(w,h)/2 - inset; on a shape
// shrunk below the inset that goes negative, and SVG drops a
// <circle> with a negative `r` entirely (renders blank).
function posR(v) {
...
const BPMN = {
  bpmnStartEvent:    { name: 'Start Event',       draw: (w, h) => `<circle cx="${w / 2}" cy="${h / 2}" r="${posR(Math.min(w, h) / 2 - 2)}" />` },
  bpmnIntermediate:  { name: 'Intermediate Event',draw: (w, h) => {
    const r = posR(Math.min(w, h) / 2 - 2);
    return `<circle cx="${w / 2}" cy="${h / 2}" r="${r}" />` +
           `<circle cx="${w / 2}" cy="${h / 2}" r="${posR(r - 4)}" fill="none" />`;
  } },
...
const NETWORK = {
  ...
  netRouter:         { name: 'Router',            draw: (w, h) => {
    const cx = w / 2, cy = h / 2, r = posR(Math.min(w, h) / 2 - 2);
...
const UML = {
  ...
  umlFinalState: { name: 'Final State', draw: (w, h) => {
    ...
    return `<circle cx="${w / 2}" cy="${h / 2}" r="${r}" fill="none" />` +
           `<circle cx="${w / 2}" cy="${h / 2}" r="${posR(r - 6)}" />`;
  } },
...
// ---------- Notes & callouts (1 for Wave 1) ----------

const NOTES = {
  stickyNote: { name: 'Sticky Note', draw: (w, h) =>
    `<path d="M0,0 L${w - 16},0 L${w},16 L${w},${h} L0,${h} Z" />` +
    `<path d="M${w - 16},0 L${w - 16},16 L${w},16" fill="none" />` },
};

// ---------- Engineering / mechanical (5) ----------

const ENGINEERING = {
  engGear: { name: 'Gear', draw: (w, h) => {
  ...
  engSpring: { name: 'Spring', draw: (w, h) => {
    const mid = h / 2;
    let p = `<line x1="0" y1="${mid}" x2="${w * 0.15}" y2="${mid}" />`;
    const coils = 6;
    const step = (w * 0.7) / coils;
    for (let i = 0; i < coils; i++) {
      const x0 = w * 0.15 + i * step;
      p += `<line x1="${x0}" y1="${mid + h * 0.25}" x2="${x0 + step / 2}" y2="${mid - h * 0.25}" />` +
           `<line x1="${x0 + step / 2}" y1="${mid - h * 0.25}" x2="${x0 + step}" y2="${mid + h * 0.25}" />`;
    }
    p += `<line x1="${w * 0.85}" y1="${mid}" x2="${w}" y2="${mid}" />`;
    return p;
  } },
  engLever: { name: 'Lever', draw: (w, h) =>
    `<line x1="${w * 0.1}" y1="${h * 0.8}" x2="${w * 0.9}" y2="${h * 0.2}" stroke-width="3" />` +
    `<polygon points="${w * 0.45},${h} ${w * 0.55},${h} ${w / 2},${h * 0.85}" />` },
  engPulley: { name: 'Pulley', draw: (w, h) => {
    const cx = w / 2, cy = h / 2, r = Math.min(w, h) * 0.35;
    return `<circle cx="${cx}" cy="${cy}" r="${r}" />` +
           `<circle cx="${cx}" cy="${cy}" r="${r * 0.15}" />` +
           `<line x1="${cx - r}" y1="${cy}" x2="0" y2="${cy}" />` +
           `<line x1="${cx + r}" y1="${cy}" x2="${w}" y2="${cy}" />`;
  } },
  engBolt: { name: 'Bolt', draw: (w, h) => {
    // hex head + threaded shaft
    const headW = w * 0.35;
    return `<polygon points="0,${h * 0.3} ${headW * 0.25},${h * 0.1} ${headW * 0.75},${h * 0.1} ${headW},${h * 0.3} ${headW},${h * 0.7} ${headW * 0.75},${h * 0.9} ${headW * 0.25},${h * 0.9} 0,${h * 0.7}" />` +
           `<rect x="${headW}" y="${h * 0.35}" width="${w - headW}" height="${h * 0.3}" />` +
           // thread strokes
           [0.45, 0.55, 0.65, 0.75, 0.85].map((f) => `<line x1="${w * f}" y1="${h * 0.35}" x2="${w * f}" y2="${h * 0.65}" />`).join('');
  } },
};
...
const CLOUD = {
  cloudGeneric: { name: 'Cloud', draw: (w, h) =>
    `<path d="M${w * 0.2},${h * 0.7} Q0,${h * 0.7} ${w * 0.1},${h * 0.45} Q${w * 0.05},${h * 0.2} ${w * 0.3},${h * 0.3} Q${w * 0.4},${h * 0.1} ${w * 0.55},${h * 0.2} Q${w * 0.75},${h * 0.1} ${w * 0.8},${h * 0.35} Q${w},${h * 0.4} ${w * 0.9},${h * 0.65} Q${w * 0.95},${h * 0.85} ${w * 0.7},${h * 0.8} Q${w * 0.5},${h * 0.95} ${w * 0.35},${h * 0.85} Q${w * 0.15},${h * 0.9} ${w * 0.2},${h * 0.7} Z" />` },
  cloudServer: { name: 'Cloud Server', draw: (w, h) =>
    `<rect x="${w * 0.15}" y="${h * 0.15}" width="${w * 0.7}" height="${h * 0.7}" rx="6" />` +
    `<rect x="${w * 0.22}" y="${h * 0.25}" width="${w * 0.56}" height="${h * 0.12}" fill="none" />` +
    `<rect x="${w * 0.22}" y="${h * 0.45}" width="${w * 0.56}" height="${h * 0.12}" fill="none" />` +
    `<circle cx="${w * 0.78}" cy="${h * 0.31}" r="2" />` },
  cloudDb: { name: 'Cloud Database', draw: (w, h) =>
    `<ellipse cx="${w / 2}" cy="${h * 0.18}" rx="${w / 2 - 4}" ry="${h * 0.15}" />` +
    `<path d="M2,${h * 0.18} L2,${h * 0.82} A${w / 2 - 2},${h * 0.15} 0 0 0 ${w - 2},${h * 0.82} L${w - 2},${h * 0.18}" fill="none" />` },
  cloudLambda: { name: 'Lambda Function', draw: (w, h) => {
    const cx = w / 2, cy = h / 2;
    return `<rect x="${w * 0.1}" y="${h * 0.1}" width="${w * 0.8}" height="${h * 0.8}" rx="6" />` +
           `<path d="M${cx - 12},${cy - 12} L${cx},${cy + 12} L${cx + 12},${cy - 12} M${cx - 6},${cy} L${cx + 6},${cy}" fill="none" stroke-width="2" />`;
  } },
  cloudQueue: { name: 'Message Queue', draw: (w, h) =>
    `<rect x="0" y="${h * 0.25}" width="${w}" height="${h * 0.5}" />` +
    [0.2, 0.4, 0.6, 0.8].map((f) => `<line x1="${w * f}" y1="${h * 0.25}" x2="${w * f}" y2="${h * 0.75}" />`).join('') },
  cloudContainer: { name: 'Container', draw: (w, h) =>
    `<rect x="${w * 0.05}" y="${h * 0.2}" width="${w * 0.9}" height="${h * 0.6}" />` +
    `<line x1="${w * 0.05}" y1="${h * 0.35}" x2="${w * 0.95}" y2="${h * 0.35}" />` +
    `<rect x="${w * 0.15}" y="${h * 0.45}" width="${w * 0.25}" height="${h * 0.25}" fill="none" />` +
    `<rect x="${w * 0.45}" y="${h * 0.45}" width="${w * 0.4}" height="${h * 0.25}" fill="none" />` },
};

// ---------- Workflow / process (6) ----------

const WORKFLOW = {
  wfInput: { name: 'Input', draw: (w, h) =>
    `<polygon points="${w * 0.15},0 ${w},0 ${w * 0.85},${h} 0,${h}" />` },
  wfOutput: { name: 'Output', draw: (w, h) =>
    `<polygon points="0,0 ${w * 0.85},0 ${w},${h} ${w * 0.15},${h}" />` },
  wfTask: { name: 'Task', draw: (w, h) => `<rect x="0" y="0" width="${w}" height="${h}" rx="6" />` },
  wfMilestone: { name: 'Milestone', draw: (w, h) =>
    `<polygon points="${w / 2},0 ${w},${h / 2} ${w / 2},${h} 0,${h / 2}" />` +
    `<polygon points="${w / 2},${h * 0.15} ${w * 0.85},${h / 2} ${w / 2},${h * 0.85} ${w * 0.15},${h / 2}" fill="none" />` },
  wfGateway: { name: 'Gateway', draw: (w, h) =>
    `<polygon points="${w / 2},0 ${w},${h / 2} ${w / 2},${h} 0,${h / 2}" />` +
    `<path d="M${w * 0.3},${h * 0.3} L${w * 0.7},${h * 0.7} M${w * 0.7},${h * 0.3} L${w * 0.3},${h * 0.7}" stroke-width="3" />` },
  wfSubProcess: { name: 'Sub-Process', draw: (w, h) =>
...
const ARROWS = {
  arrSingleR: { name: 'Arrow Right', draw: (w, h) =>
    `<polygon points="0,${h * 0.3} ${w * 0.7},${h * 0.3} ${w * 0.7},${h * 0.1} ${w},${h / 2} ${w * 0.7},${h * 0.9} ${w * 0.7},${h * 0.7} 0,${h * 0.7}" />` },
  arrDouble: { name: 'Double Arrow', draw: (w, h) =>
    `<polygon points="0,${h / 2} ${w * 0.2},${h * 0.1} ${w * 0.2},${h * 0.3} ${w * 0.8},${h * 0.3} ${w * 0.8},${h * 0.1} ${w},${h / 2} ${w * 0.8},${h * 0.9} ${w * 0.8},${h * 0.7} ${w * 0.2},${h * 0.7} ${w * 0.2},${h * 0.9}" />` },
  arrCurved: { name: 'Curved Arrow', draw: (w, h) =>
    `<path d="M${w * 0.1},${h * 0.85} Q${w * 0.5},${h * 0.1} ${w * 0.9},${h * 0.45} L${w * 0.95},${h * 0.35} L${w * 0.85},${h * 0.6} L${w * 0.7},${h * 0.55}" fill="none" stroke-width="3" />` +
    `<polygon points="${w * 0.85},${h * 0.6} ${w * 0.95},${h * 0.35} ${w * 0.7},${h * 0.55}" />` },
  arrUTurn: { name: 'U-Turn', draw: (w, h) =>
    `<path d="M${w * 0.2},${h * 0.8} L${w * 0.2},${h * 0.5} Q${w * 0.2},${h * 0.2} ${w * 0.6},${h * 0.2} L${w * 0.6},${h * 0.35} L${w * 0.85},${h * 0.15} L${w * 0.6},0 L${w * 0.6},${h * 0.1} Q${w * 0.05},${h * 0.1} ${w * 0.05},${h * 0.5} L${w * 0.05},${h * 0.8} Z" />` },
  arrBranch: { name: 'Branching Arrow', draw: (w, h) =>
    `<polygon points="0,${h * 0.45} ${w * 0.55},${h * 0.45} ${w * 0.55},${h * 0.25} ${w},${h / 2} ${w * 0.55},${h * 0.75} ${w * 0.55},${h * 0.55} 0,${h * 0.55}" />` +
    `<polygon points="${w * 0.55},0 ${w * 0.7},0 ${w * 0.7},${h * 0.2} ${w * 0.85},${h * 0.05} ${w * 0.7},${h * 0.3} ${w * 0.55},${h * 0.3}" />` },
  arr4Way: { name: '4-Way Arrow', draw: (w, h) =>
    `<polygon points="${w * 0.4},${h * 0.15} ${w * 0.6},${h * 0.15} ${w * 0.6},${h * 0.4} ${w * 0.85},${h * 0.4} ${w * 0.85},${h * 0.6} ${w * 0.6},${h * 0.6} ${w * 0.6},${h * 0.85} ${w * 0.4},${h * 0.85} ${w * 0.4},${h * 0.6} ${w * 0.15},${h * 0.6} ${w * 0.15},${h * 0.4} ${w * 0.4},${h * 0.4}" />` +
    `<polygon points="${w / 2},${h * 0.05} ${w * 0.6},${h * 0.15} ${w * 0.4},${h * 0.15}" />` +
    `<polygon points="${w / 2},${h * 0.95} ${w * 0.6},${h * 0.85} ${w * 0.4},${h * 0.85}" />` +
    `<polygon points="${w * 0.05},${h / 2} ${w * 0.15},${h * 0.4} ${w * 0.15},${h * 0.6}" />` +
    `<polygon points="${w * 0.95},${h / 2} ${w * 0.85},${h * 0.4} ${w * 0.85},${h * 0.6}" />` },
};
...
export const STENCILS = {
  ...
  ...mark('Notes', NOTES),
  ...mark('Engineering', ENGINEERING),
  ...mark('Cloud', CLOUD),
  ...mark('Workflow', WORKFLOW),
  ...mark('Arrows', ARROWS),
};

// Pseudo-stencil for imported images (e.g. an opened SVG). It has no
// drawer category — stencilsByCategory() skips unknown categories —
// and renderers special-case `stencil === 'image'` to draw the
// shape's `src` instead of calling draw().
STENCILS.image = {
  id: 'image',
  name: 'Image',
  category: 'Image',
  draw(w, h) {
    return `<rect x="0" y="0" width="${w}" height="${h}" fill="none" stroke="none" />`;
  },
};
...
export const CATEGORIES = [
  ...
  'Electrical', 'Floor Plan', 'Mind Map', 'Notes',
  'Engineering', 'Cloud', 'Workflow', 'Arrows',
];
...
export function stencilsByCategory() {
  ...
  for (const s of Object.values(STENCILS)) {
    // Skip stencils whose category isn't a drawer category (e.g. the
    // internal `image` pseudo-stencil).
