...
export function saveVsdx(diagram) {
  ...
  // Lossless round-trip part. The OOXML schema above can't represent
  // every RodmanVision field — shape hyperlinks / comments / data,
  // connector waypoints / routeStyle / lineStyle / labelPos / text2,
  // per-end arrow kinds, groups, layers, themes. We additionally
  // stash the entire diagram model as a non-standard JSON part.
  // Real Visio ignores an unreferenced part it doesn't recognise;
  // loadVsdx (below) prefers it, so a RodmanVision save → load
  // cycle is exact. `[Content_Types].xml` declares the json
  // extension so the package stays OPC-valid.
  if (diagram) {
    files.push({
      name: 'visio/rodman-vision.json',
      data: enc.encode(JSON.stringify(diagram)),
    });
  }
...
export async function loadVsdx(arrayBuffer) {
  ...
  // Lossless path: a VSDX written by saveVsdx carries the full
  // RodmanVision model as visio/rodman-vision.json. Prefer it so a
  // save → load cycle is exact. Real Visio files (and pre-v1
  // RodmanVision exports) lack this part and fall through to the
  // OOXML parser below. A corrupt embedded part also falls through.
  const embedded = files['visio/rodman-vision.json'];
  if (embedded) {
    try {
      const model = JSON.parse(dec.decode(embedded));
      if (model && Array.isArray(model.pages)) return model;
    } catch (_) {
      // Fall through to OOXML parsing.
    }
  }
