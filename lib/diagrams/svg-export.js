...
export function renderShape(shape, layerOpacity = 1) {
  ...
  // Image shapes (e.g. an imported SVG) embed their source directly.
  if (shape.stencil === 'image' && shape.src) {
    const op = (shape.opacity ?? 1) * layerOpacity;
    const href = escAttr(shape.src);
    return `<g transform="${transform}" opacity="${op}">` +
      `<image x="0" y="0" width="${shape.w}" height="${shape.h}" ` +
      `preserveAspectRatio="xMidYMid meet" href="${href}" /></g>`;
  }
