// FileMerger: video merge engine. Ported from MP4 Merger
// (https://github.com/Rodman-Ai/mp4merger, src/merge.ts) by stripping the
// TypeScript types. Decode/encode runs on WebCodecs via the vendored
// Mediabunny bundle (lib/video/vendor/mediabunny/, MPL-2.0).
import { ALL_FORMATS, AudioSample, AudioSampleSink, AudioSampleSource, BlobSource, BufferTarget, Input, Mp4OutputFormat, Output, Quality, StreamTarget, VideoSampleSink, VideoSampleSource, canEncodeVideo, getFirstEncodableAudioCodec } from '../lib/video/vendor/mediabunny/mediabunny.min.mjs';
...
async function canDecodeInHardware(video) {
...
* encoder.
*/
export function makeQuality(quality) {
...
export async function probeEncoders(width, height, frameRate, quality) {
  const candidates = ['avc', 'hevc', 'av1', 'vp9'];
  const q = makeQuality(quality);
  const probe = (codec, hardwareAcceleration) => canEncodeVideo(codec, { width, height, frameRate, quality: q, hardwareAcceleration }).catch(() => false);
  const results = await Promise.all(candidates.map(async (codec) => {
...
*/
export async function mergeClips(clips, sink, opts) {
  ...
  if (hasAudio && !audioCodec)
    throw new Error('This browser cannot encode AAC or Opus audio');
  const target = sink.kind === 'file' ? new StreamTarget(sink.writable, { chunked: true }) : new BufferTarget();
  ...
  try {
    ...
    for (const [clipIndex, clip] of clips.entries()) {
      ...
      const report = (t) => opts.onProgress({ done: clipOffset + t, total, clipIndex, clipDone: t, framesEncoded });
      report(0);
      const tasks = [pumpVideo(clip, clipOffset, videoSource, opts, report)];
      if (audioSource) {
        tasks.push(clip.audio
    ...
    if (sink.kind === 'file')
      await sink.writable.abort().catch(() => { });
    throw err;
  }
  opts.onProgress({
    done: total,
...
async function pumpVideo(clip, offset, source, opts, onTime) {
...
async function pumpAudio(clip, track, offset, source, signal) {
  ...
  const emit = async (planes) => {
    ...
    for (let ch = 0; ch < AUDIO_CHANNELS; ch++)
      data.set(planes[ch].subarray(0, frames), ch * frames);
  ...
  for await (const sample of sink.samples()) {
    try {
      ...
      if (!resampler) {
        ...
        if (lead > 0)
          await emit(silence(Math.min(lead, totalFrames)));
      }
...
function toStereoPlanes(sample) {
  ...
  process(input) {
    ...
    for (let k = 0; k < count; k++, p += this.step) {
      ...
      for (let ch = 0; ch < buf.length; ch++)
        out[ch][k] = buf[ch][i] * (1 - f) + buf[ch][i + 1] * f;
    }
...
async function pumpSilence(offset, duration, source, signal) {
