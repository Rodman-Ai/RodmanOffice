import { CanvasSink } from '../lib/video/vendor/mediabunny/mediabunny.min.mjs';
import { mergeClips, probeClip, probeEncoders } from './video-merge.js';
// FileMerger: video tab. Ported from MP4 Merger
// (https://github.com/Rodman-Ai/mp4merger, src/main.ts) by stripping the
// TypeScript types; logic is unchanged apart from drag-and-drop, which
// app.js now routes for both tabs.
...
const QUALITY_BPP = { 'very-high': 0.15, high: 0.1, medium: 0.06, low: 0.035 };
function fmtMbps(bps) {
  return `${(bps / 1e6).toFixed(bps < 10e6 ? 1 : 0)} Mbps`;
}
function roundBitrate(bps) {
...
function sourceBitrate(width, height, fps) {
  const first = firstClip();
  if (!first || !first.bitrate)
    return null;
...
export async function addFiles(files) {
  ...
  await Promise.all(added.map(async (entry) => {
    try {
      ...
      if (frame)
        entry.thumb = await thumbUrl(frame.canvas);
    }
    catch (err) {
...
function render() {
  list.replaceChildren(...entries.map((entry, idx) => {
    ...
    ctrl.append(mk('↑', 'Move up', () => move(entry.id, -1), idx === 0), mk('↓', 'Move down', () => move(entry.id, 1), idx === entries.length - 1), mk('✕', 'Remove', () => removeEntry(entry.id)));
...
function currentQuality() {
  ...
  if (v === 'custom')
    return Math.max(100_000, Math.round(Number(bitrateInput.value) * 1_000_000));
  if (v === 'source') {
...
function updateOptionLabels() {
  ...
  set(qualitySel, 'source', src ? `First clip (${fmtMbps(src)}${scaled ? ', scaled' : ''})` : 'Match first clip');
  const names = { 'very-high': 'Very high', high: 'High', medium: 'Medium', low: 'Low' };
...
async function refreshCodecs() {
  ...
  encoders = found;
  codecSel.replaceChildren(...encoders.map((e) => {
...
function syncAccel() {
  const enc = selectedEncoder();
  const requireOpt = accelSel.querySelector('option[value="require"]');
  requireOpt.disabled = !enc?.hardware;
  if (!enc?.hardware)
    accelSel.value = 'auto';
  else if (!accelSel.dataset.userSet)
    accelSel.value = 'require';
  renderHw();
  updateMergeButton();
}
...
// Drag-and-drop is routed by app.js, which sends videos here and PDFs to the PDF tab.
...
mergeBtn.addEventListener('click', () => {
  ...
  const cpuParts = [];
  if (enc && !enc.hardware)
    cpuParts.push(`encoding (${CODEC_LABELS[enc.codec]} has no GPU encoder at this size)`);
  if (cpuDecode)
    cpuParts.push(`decoding for ${cpuDecode} clip${cpuDecode === 1 ? '' : 's'}`);
  if (cpuParts.length) {
...
async function startMerge() {
  ...
  let encodeFps = 0;
  function paintProgress(p, now = performance.now()) {
    ...
    for (const li of list.querySelectorAll('li.clip')) {
      const id = Number(li.dataset.id);
      li.classList.toggle('active', id === activeClipId);
      const b = li.querySelector('.clip-bar');
      if (b)
        b.style.width = `${(clipProgress.get(id) ?? 0) * 100}%`;
    }
    const cell = (value, label) => `<div><b>${value}</b><span>${label}</span></div>`;
    stats.innerHTML =
  ...
  try {
    ...
    const frames = final?.framesEncoded ?? 0;
    const gpuDecodeAll = clips.every((c) => c.hwDecode);
    const accelNote = acceleration === 'require'
    ...
    if (running.signal.aborted) {
      ...
      const hint = acceleration === 'require'
